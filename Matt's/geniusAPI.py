...
### The following code is a modified version of the "genius.py" script from the LyricsGenius library. ###
### The authors of SpotiFeels do not take any credit for code that is shared between this script and the "genius.py" script. ###
...
from lyricsgenius.utils import clean_str, safe_unicode
import unicodedata
...
class Genius(API, PublicAPI):
    ...
    def search_song(self, title=None, artist="", song_id=None,
                    get_full_info=True):
        title = re.sub(r'[^\w\s]', ' ', title.replace(' & ', ' and '))
        artist = re.sub(r'[^\w\s]', ' ', artist.replace(' & ', ' and '))
        ...
        # # Reject song if 'lyrics' not in URL
        url_split = str(song_info['url'])[19:].lower().split('-')
        # print(url_split)
        if 'lyrics' not in url_split:
            if self.verbose:
                print('#################')
                print('\'lyrics\' not contained in URL. Rejecting.')
                print(title, artist)
                print(song_info['url'])
                print('#################')
            return None

        # Reject song if URL doesn't match search terms
        artist_split = artist.lower().split(' and ')
        song_split = title.lower().split(' ')  # Problem with punctuation in split song title terms
        # print(song_split)
        i = False
        j = False
        for a in artist_split:
            for b in a.split():
                if b in url_split:
                    i = True
        for c in song_split:
        ...
        if not i:
            print('#################')
            print('Artist not found in the URL. Rejecting.')
            print(title, artist)
            print(artist_split)
            print(song_info['url'])
            print('#################')
        ...
        if not j:
            print('#################')
            print('Song title not found in the URL. Rejecting.')
            print(title, artist)
            print(song_split)
            print(song_info['url'])
            print('#################')
        ...
        # Reject song if song is a "This Is [artist]" list
        if 'this' in url_split and 'is' in url_split and 'this' not in song_split and 'is' not in song_split:
            print('#################')
            print('Found a \'This Is [artist]\' list. Rejecting.')
            print(title, artist)
            # print(song_split)
            print(song_info['url'])
            print('#################')
